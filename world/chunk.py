...
class Chunk:
    ...
    def __init__(self, w_pos, seed, blocks_data=None):
        ...
        if blocks_data is None:
            self._blocks = self._generator.gen_chunk_blocks(self._w_pos)
        else:
    ...
    def _update_colliders(self):
        self.colliders = Colliders()
        for block_w_pos in self._blocks:
            if not (block_w_pos[0]-1, block_w_pos[1]) in self._blocks:
                self.colliders.left.append(block_w_pos)
            if not (block_w_pos[0]+1, block_w_pos[1]) in self._blocks:
                self.colliders.right.append(block_w_pos)
            if not (block_w_pos[0], block_w_pos[1]-1) in self._blocks:
    ...
    def _draw(self):
        ...
        blit_sequence = []
        for block_w_pos, block in self._blocks.items():
    ...
    def _break_block(self, block_w_pos):
        self._blocks.pop(block_w_pos, None)
    ...
    def _place_block(self, block_w_pos, material):
        ...
        self._blocks[block_w_pos] = block
