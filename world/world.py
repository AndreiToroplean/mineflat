...
from core.funcs import w_to_chunk_vec, w_to_pix_shift, w_to_chunk_to_w_vec
...
class World:
    ...
    def _update_chunk_view(self, camera):
        ...
        new_chunk_view = ChunkView(
            w_to_chunk_vec(camera.w_view.pos_0),
            w_to_chunk_vec(camera.w_view.pos_1),
            )
    ...
    def _load_chunks(self):
        ...
        self._chunks_visible = {}
        for chunk_w_pos_x in range(self._max_view.pos_0.x, self._max_view.pos_1.x, CHUNK_SIZE.x):
            for chunk_w_pos_y in range(self._max_view.pos_0.y, self._max_view.pos_1.y, CHUNK_SIZE.y):
                chunk_w_pos = WorldVec(chunk_w_pos_x, chunk_w_pos_y)
    ...
    def _chunk_pos_to_pix_shift(self, chunk_w_pos):
        max_view_w_shift = WorldVec(*(pos - shift for pos, shift in zip(chunk_w_pos, self._max_view.pos_0)))
    ...
    def _draw_max_surf(self):
        ...
        for chunk_w_pos, chunk in self._chunks_visible.items():
            pix_shift = self._chunk_pos_to_pix_shift(chunk_w_pos)
    ...
    def _resize_max_surf(self, camera):
        max_surf_pix_size = tuple(
            (dim + 2) * pix
            for dim, pix in zip(w_to_chunk_vec(camera.w_size), CHUNK_PIX_SIZE)
            )
    ...
    def _get_chunk_at_pos(self, pos):
        chunk_w_pos = w_to_chunk_to_w_vec(pos)
        try:
